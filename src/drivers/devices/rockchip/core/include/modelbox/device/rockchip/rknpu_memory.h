...
#ifndef MODELBOX_RKNPU_MEMORY_H_
#define MODELBOX_RKNPU_MEMORY_H_
...
class RKNPUMemory : public DeviceMemory {
 public:
  RKNPUMemory(const std::shared_ptr<Device> &device,
...
class RKNPUMemoryManager;
class RKNPUMemoryPool : public MemoryPoolBase {
 public:
  RKNPUMemoryPool(RKNPUMemoryManager *mem_manager);
  virtual ~RKNPUMemoryPool();
  Status Init();
 ...
 private:
  RKNPUMemoryManager *mem_manager_;
  std::shared_ptr<TimerTask> flush_timer_;
};

class RKNPUMemoryManager : public DeviceMemoryManager {
 public:
  RKNPUMemoryManager(const std::string &device_id);
  virtual ~RKNPUMemoryManager();

  Status Init();

  /* *
   * @brief Create a rknpu memory container
  ...
  /* *
   * @brief Implement by rknpu device, alloc memory
  ...
  /**
   * @brief Implement by rknpu device, copy data from src to dest
  ...
  /* *
   * @brief Copy memory between rknpu device and host
