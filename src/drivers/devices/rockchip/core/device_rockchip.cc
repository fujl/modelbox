...
RockChipFactory::DeviceProbe() {
  RKNNDevs::Instance().SetDeviceType();
...
void RKNNDevs::SetDeviceType() {
  ...
  while (getline(dev_file, strLine)) {
    if (strLine.find("rk3399pro") != std::string::npos) {
      type_ = RKNN_DEVICE_TYPE_RK3399PRO;
      break;
    }

    if (strLine.find("rk356") != std::string::npos) {
      type_ = RKNN_DEVICE_TYPE_RK356X;
      break;
    }

    if (strLine.find("rk358") != std::string::npos) {
      type_ = RKNN_DEVICE_TYPE_RK358X;
      break;
    }

    if (strLine.find("rv110") != std::string::npos) {
      type_ = RKNN_DEVICE_TYPE_RV110X;
      break;
    }
